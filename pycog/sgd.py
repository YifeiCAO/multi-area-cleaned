...
class SGD(object):
    ...
    """
    @staticmethod
    def adam(all_params, all_grads, learning_rate=0.01, b1=0.9, b2=0.999, e=1e-8, gamma=1-1e-8):
        """
        ADAM update rules
...
#        pdb.set_trace()
    ...
    def __init__(self, trainables, inputs, costs, regs, x, r, z, params, save_values,
                 extras):
        """
...
# lr is a placeholder theano scalar, that is later supplied by calling train_step with the appropriate input.
#        updates = SGD.adam(self.trainables, g, learning_rate=lr)
#        updates = SGD.adam(self.trainables, g, learning_rate=lr)

		# ADAM written out manually
        b1=0.9
        b2=0.999
        eps=1e-8
        gamma=1-1e-8
        ...
        for theta_previous, grad in zip(self.trainables, g):
            ...
            m = b1_t*m_previous + (1 - b1_t)*grad
            v = b2*v_previous + (1 - b2)*grad**2
            m_hat = m / (1-b1**t)
            v_hat = v / (1-b2**t)
            theta = theta_previous - (lr * m_hat) / (T.sqrt(v_hat) + eps)
        ...
        # Update function, # JCK NOTE: 'inputs' contains both inputs and outputs.
		...
		# DEBUGGING, BE SURE TO REMOVE!!!!!!!!!
        #self.f_cost = theanotools.function(inputs, [costs[0] + regs] + costs[1:] + [z] + [r] + [x])
    ...
    def train(self, gradient_data, validation_data, savefile):
        ...
        if os.path.isfile(savefile):
            ...
            # pdb.set_trace() # to hack in and change the best cost.
			...
			# JCK 2017-08-25 - CHANGING INIT here for DEBUGGING.
            #bps = best['params']
            #del bps[4] # removes bout
            #for i, j in zip(self.trainables, best['params']):
            #    i.set_value(j)

            print(("[ {}.SGD.train ] Recovered saved model,"
                   " continuing from iteration {}.").format(THIS, first_iter))

            # note, best parameters stored in best['params']
            ...
            for iter in range(first_iter, 1+self.p['max_iter']):
                if iter % checkfreq == 1:
                    ...
                    if costs[0] < best['cost']:
                        ...
                        base, ext = os.path.splitext(savefile)
                        dump(base + '_' + str(iter) + '_' + str(round(costs[0], 6)) + ext, save)
