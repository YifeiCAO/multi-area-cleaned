...
class Trainer(object):
  ...
  def __init__(self, params, floatX=theano.config.floatX):
    ...
    if self.p['distribution_rec'] is None:
      if self.p['ei'] is not None:
        if cfg_mk['gamma_rec']:
          self.p['distribution_rec'] = 'gamma'  # gamma
        else:
    ...
    # TODO: write a function for this that takes in number of units
  ...
  def train(self, savefile, task, recover=True):
    ...
    # write down the number of inputs, units, and outputs
    settings['Nin/N/Nout'] = '{}/{}/{}'.format(Nin, N, Nout)

    # if dale's law is used, write down the number of Exc and Inh channels
    if self.p['ei'] is not None:
      Nexc = len(np.where(self.p['ei'] > 0)[0])
      Ninh = len(np.where(self.p['ei'] < 0)[0])
      settings['Dale\'s law'] = 'E/I = {}/{}'.format(Nexc, Ninh)

# JCK 2016-08-26 COMMENTED THIS OUT AS I DONT WANT THIS CONSTRAINT ON THEINPUTS
# JCK 2018-08-23 RECOMMENTED THIS BACK IN AS THESE PROJ SHOULD BE POSITIVE.

# MK 2020-03-24
      if cfg_mk['positive_ic']:
        if Nin > 0:
    ...
    # this part scales Wrec_0 to have a spectral radius of 1.
    ...
    # declare the theano shared variables
    if Nin > 0:
...
# JCK 2016-08-26
# THIS MAKES ONLY EXCITATORY UNITS ON THE INPUT!  I've commented it out as I want Dale's Law but don't want the excitatory output.
# JCK 2018-08-23 -- put this back in
      if Nin > 0:
        if cfg_mk['make_positive']:
      ...
      # MK 2020-03-20
      # hack to put a mask on the feedforward connectivity matrix to test Dale's law
      Wff1_np = np.ones((300, 300))
      Wff2_np = np.ones((300, 300))
      idx1 = np.arange(300)
      idx2 = np.arange(300)
      # idx1 = np.arange(120)
      # idx2 = np.arange(120, 240)

      rng = np.random.RandomState(1000)
      for i in idx1:
        for j in idx2:
          if (rng.uniform(size=1) < 0.5):
            Wff1_np[i, j] = -1
          # if (rng.uniform(size=1) < 0.5):
          #   Wff2_np[j, i] = -1
      Wff1 = theanotools.shared(Wff1_np, name='Wff1')
      # Wff2 = theanotools.shared(Wff2_np, name='Wff2')

      if cfg_mk['random_mask_wrec']:
        Wrec_ = Wrec_ * Wff1
      # Wrec_ = Wrec_ * Wff2


# JCK 2016-08-23
# THIS MAKES ONLY EXCITATORY UNITS ON THE OUTPUT!  I've commented it out as I want Dale's Law but don't want the excitatory output.
# JCK 2018-08-23 -- put this back in
      if cfg_mk['make_positive']:
      ...
      Wout_np = np.ones((2, 300))
      for i in range(2):
        for j in range(300):
          if (rng.uniform(size=1) < 0.5):
            Wout_np[i, j] = -1
      Wout_mask = theanotools.shared(Wout_np, name='Wout_mask')
      if cfg_mk['random_mask_wout']:
        Wout_ = Wout_ * Wout_mask

      Win_np = np.ones((300, 4))
      for i in range(Win_np.shape[0]):
        for j in range(Win_np.shape[1]):
          if (rng.uniform(size=1) < 0.5):
            Win_np[i, j] = -1
      Win_mask = theanotools.shared(Win_np, name='Win_mask')
      if cfg_mk['random_mask_win']:
        Win_ = Win_ * Win_mask

    make_positive = abs
    if cfg_mk['make_positive_output']:
      Wout_ = make_positive(Wout_)
# JCK 2016-12-16
# Constrains speed to have only positive projections.
#        if self.p['ei_out'] is not None:
#            if self.p['ei_positive_func'] == 'abs':
#                settings['E/I positivity function'] = 'absolute value'
#                make_positive = abs
#            elif self.p['ei_positive_func'] == 'rectify':
...
#                raise ValueError("Unknown ei_positive_func.")
#
#            #Wout_ = make_positive(Wout_)*self.p['ei_out']
#            # hack for now
#            Wo_s = make_positive(Wout_[0,:])
#            Wout_ = T.set_subtensor(Wout_[0,:], Wo_s)
    ...
    # default is defined in defaults.py
    ...
    # JCK / Penalty on the final hand position
    # here, z has dimensions (time, trials, outputs)
#        pdb.set_trace()
    # loss += T.sum((abs(T.sum(z, axis=0)) / 100 * self.p['dt'] - 90.1635)**2) / 5e4 # the 1e3 is to compare to masknorm
    ...
    sgd = SGD(trainables, inputs, costs, regs, x, r, z, self.p, save_values,
