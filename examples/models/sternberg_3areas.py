...
# Define three areas: Sensory (encoding) -> Working Memory -> Decision
EXC_SENSORY = EXC[:Nexc // 3]
INH_SENSORY = INH[:Ninh // 3]
EXC_MEMORY = EXC[Nexc // 3:2 * Nexc // 3]
INH_MEMORY = INH[Ninh // 3:2 * Ninh // 3]
EXC_DECISION = EXC[2 * Nexc // 3:]
INH_DECISION = INH[2 * Ninh // 3:]

# Input connectivity: only sensory area receives input
Cin = np.zeros((N, Nin))
...
# Build recurrent connectivity matrix with hierarchical structure
Crec = np.zeros((N, N))

# Sensory area (receives input and projects to memory)
for i in EXC_SENSORY:
    Crec[i, EXC_SENSORY] = 1
    Crec[i, i] = 0
    Crec[i, EXC_MEMORY] = 1 * (rng.uniform(size=len(EXC_MEMORY)) < fb_prop)  # back projections
    Crec[EXC_MEMORY, i] = 1 * (rng.uniform(size=len(EXC_MEMORY)) < ff_prop)  # forward projections
    Crec[i, INH_SENSORY] = np.sum(Crec[i, EXC_SENSORY]) / len(INH_SENSORY)

# Memory area (maintains information and projects to decision)
for i in EXC_MEMORY:
    Crec[i, EXC_MEMORY] = 1
    Crec[i, i] = 0
    Crec[i, EXC_DECISION] = 1 * (rng.uniform(size=len(EXC_DECISION)) < fb_prop)  # back projections
    Crec[EXC_DECISION, i] = 1 * (rng.uniform(size=len(EXC_DECISION)) < ff_prop)  # forward projections
    Crec[i, INH_MEMORY] = np.sum(Crec[i, EXC_MEMORY]) / len(INH_MEMORY)

# Decision area (produces output)
for i in EXC_DECISION:
    Crec[i, EXC_DECISION] = 1
    Crec[i, i] = 0
    Crec[i, INH_DECISION] = np.sum(Crec[i, EXC_DECISION]) / len(INH_DECISION)
...
for i in INH_MEMORY:
    Crec[i, EXC_MEMORY] = 1
    Crec[i, INH_MEMORY] = np.sum(Crec[i, EXC_MEMORY]) / (len(INH_MEMORY) - 1)
    Crec[i, i] = 0
for i in INH_DECISION:
    Crec[i, EXC_DECISION] = 1
    Crec[i, INH_DECISION] = np.sum(Crec[i, EXC_DECISION]) / (len(INH_DECISION) - 1)
    Crec[i, i] = 0

Crec /= np.linalg.norm(Crec, axis=1)[:, np.newaxis]

# Output connectivity: only read from decision area
